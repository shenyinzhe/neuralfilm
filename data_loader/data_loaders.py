...
class DataLoader(BaseDataLoader):
    def __init__(self, data_dir, batch_size, augment: bool=True, shuffle=True, validation_split=0.0, num_workers=1, training=True):
...
class HasselbladDataset(Dataset):
    def __init__(self, data_dir, augment: bool=True):
        ...
        self.images = sorted(os.listdir(self.input_dir), key=lambda x:int(x.split(".")[0]))
        self.target = sorted(os.listdir(self.label_dir), key=lambda x:int(x.split(".")[0]))
